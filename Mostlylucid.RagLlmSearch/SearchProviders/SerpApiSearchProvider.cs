...
public class SerpApiSearchProvider : ISearchProvider
{
    ...
    public async Task<SearchResponse> SearchAsync(string query, int maxResults = 5, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var url = $"{BaseUrl}?engine={_settings.Engine}&q={encodedQuery}&location={encodedLocation}&api_key={_settings.ApiKey}&num={maxResults}";

            var httpResponse = await _httpClient.GetAsync(url, cancellationToken);
            httpResponse.EnsureSuccessStatusCode();

            var serpResponse = await httpResponse.Content.ReadFromJsonAsync<SerpApiResponse>(cancellationToken: cancellationToken);
...
internal class SerpApiResponse
{
    ...
    public SerpApiMetadata? SearchMetadata { get; set; }

    [JsonPropertyName("organic_results")]
    public List<SerpApiOrganicResult>? OrganicResults { get; set; }

    [JsonPropertyName("knowledge_graph")]
    public SerpApiKnowledgeGraph? KnowledgeGraph { get; set; }

    [JsonPropertyName("answer_box")]
    public SerpApiAnswerBox? AnswerBox { get; set; }
