...
(function() {
    ...
    var MLBotD = {
        ...
        config: {
            ...
            collectInteraction: %%COLLECT_INTERACTION%%,
            timeout: %%TIMEOUT%%
        },
        ...
        setupInteractionSignals: function() {
            ...
            return function() { return interacted; };
        },
        ...
        calculateScore: function(data) {
            ...
            if (data.webdriver) { score -= 50; reasons.push('webdriver'); }
            if (data.phantom) { score -= 50; reasons.push('phantom'); }
            if (data.nightmare) { score -= 50; reasons.push('nightmare'); }
            if (data.selenium) { score -= 50; reasons.push('selenium'); }
            if (data.cdc) { score -= 40; reasons.push('cdp'); }

            // Suspicious indicators
            if (data.plugins === 0 && data.chrome) { score -= 20; reasons.push('chrome-no-plugins'); }
            if (data.outerW === 0 || data.outerH === 0) { score -= 30; reasons.push('zero-outer'); }
            if (data.innerW === data.outerW && data.innerH === data.outerH) { score -= 10; reasons.push('no-chrome-ui'); }
            if (!data.bindNative) { score -= 20; reasons.push('bind-not-native'); }
            if (data.evalLen > 0 && (data.evalLen < 30 || data.evalLen > 50)) { score -= 15; reasons.push('eval-len-weird'); }
            if (data.permissions === 'suspicious') { score -= 25; reasons.push('perm-suspicious'); }

            // Anti-tamper signals
            if (data.getBatteryNative === 0) { score -= 15; reasons.push('getBattery-wrapped'); }
            if (data.consoleDebugNative === 0) { score -= 10; reasons.push('console-wrapped'); }
            if (data.querySelectorNative === 0) { score -= 15; reasons.push('querySelector-wrapped'); }

            // Context anomalies
            if (data.isIframe === 1 && data.outerW === 0) { score -= 20; reasons.push('suspicious-iframe'); }
