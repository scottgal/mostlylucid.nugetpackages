...
Version age detection analyzes browser and OS versions in User-Agent strings to identify bots using outdated or impossible version combinations.
...
The detector fetches current browser versions from external APIs. **Works out of the box with zero configuration** - sensible fallback versions are built-in.
